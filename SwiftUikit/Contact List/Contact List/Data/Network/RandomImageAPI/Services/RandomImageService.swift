...
class RandomImageService {
    ...
    func fetchRandomImage() async throws -> RandomImageDTO? {
        ...
        let randomImageDTO = try decoder.decode([RandomImageDTO].self, from: data)
        
        if randomImageDTO.self.self == nil {
            throw NetworkError.BadResponse
        }
        
       return randomImageDTO[0]
    }
}
